...
import { IMPORT_SCOPE_CLASS } from '../importer/css/scope-imported-css.mjs';
import { breakpointCascade } from '../responsive-styles.mjs';
import { compileBlockStyles } from '../styles/compiler.mjs';
import { findBlock, findBlockLocation } from '../tree.mjs';

export const GSAP_ANIMATION_BEHAVIORS = new Set([
	'scroll-scrub',
	'stagger-sequence',
]);
export const CSS_ANIMATION_BEHAVIORS = new Set(['css-reveal']);
export let editorGsapPromise;
...
export function buildPreviewStyles(document, activeBreakpoint) {
	...
	function visit(block) {
		...
		const legacyStyleSelector =
			block.element === 'core/image'
				? `${selector} > [data-ctb-part="media"]`
				: selector;
		const hasTargetStyles =
			Object.keys(block.style?.targets || {}).length > 0;
		const fallback = previewCustomCssFallback(
			String(block.styles?.custom_css_fallback || '')
		);
		if (fallback && !hasTargetStyles) {
			base.push(
				`${legacyStyleSelector}{${fallback.replace(/;+$/, '')};}`
			);
		}
		if (hasTargetStyles) {
			const compiled = compileBlockStyles(block, 0, {
				rootSelector: '.ctb-canvas-stage',
			});
			if (compiled.css) {
				base.push(compiled.css);
			}
		}

		for (const state of hasTargetStyles
			? []
			: ['hover', 'focus', 'active']) {
			...
			if (declarations) {
				base.push(
					`${legacyStyleSelector}:${state}{${declarations}}`
				);
			}
		}

		for (const viewport of hasTargetStyles
			? []
			: breakpointCascade(activeBreakpoint)) {
			const declarations = previewDeclarations(
			...
			if (declarations) {
				responsive.push(
					`${legacyStyleSelector}{${declarations}}`
				);
			}
		}
